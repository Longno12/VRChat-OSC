...
import threading, time, datetime, random, json, os, webbrowser, math
...
class VrcOscThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.is_running:
            try:
                ...
                if current_message != last_message: self.osc_client.send_message("/chatbox/input", [current_message, True])
    ...
    def build_message(self):
        ...
        if self.config.get('module_clock'):
            line1_parts.append(f"🕒 {datetime.datetime.now().strftime('%H:%M:%S' if self.config.get('clock_show_seconds') else '%H:%M')}")
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        """Creates the necessary widgets for the TK screen."""
