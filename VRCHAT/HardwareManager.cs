...
{ 
    ...
    private IHardware _cpu, _gpu, _ram; 
    ...
    public HardwareManager() 
    {
        ...
        _computer.Open(); _cpu = _computer.Hardware.FirstOrDefault(h => h.HardwareType == HardwareType.Cpu); 
        ...
        CpuName = _cpu?.Name ?? "N/A"; GpuName = _gpu?.Name ?? "N/A"; Update(); 
    ...
    public void Update() 
    { 
        _cpu?.Update(); _gpu?.Update(); _ram?.Update(); CpuLoad = _cpu?.Sensors.FirstOrDefault(s => s.SensorType == SensorType.Load && s.Name == "CPU Total")?.Value ?? 0; CpuTemp = _cpu?.Sensors.FirstOrDefault(s => s.SensorType == SensorType.Temperature)?.Value ?? 0; GpuLoad = _gpu?.Sensors.FirstOrDefault(s => s.SensorType == SensorType.Load && s.Name == "GPU Core")?.Value ?? 0; GpuTemp = _gpu?.Sensors.FirstOrDefault(s => s.SensorType == SensorType.Temperature)?.Value ?? 0; RamUsed = _ram?.Sensors.FirstOrDefault(s => s.SensorType == SensorType.Data && s.Name == "Memory Used")?.Value ?? 0; } public void Close() => _computer.Close(); }